/**
 * Pure rules for a draw.io DIAGRAM file (YAZ-1802) — shared by main's two diagram doors, the
 * create door, the tree's head read, and the renderer's "New draw.io diagram". No DOM and no XML
 * parser: main has neither, and these rules only ever need the document's OUTLINE and the root
 * element's opening tag.
 *
 * 🔒 YAZ-1802 D3: a `.drawio` is saved as plain, uncompressed XML — `<mxfile>` → `<diagram>` →
 * `<mxGraphModel>`. A compressed page (deflate + base64 inside `<diagram>`) still OPENS; draw.io
 * rewrites it plain on the next save.
 *
 * 🔒 YAZ-1802 D7: a diagram carries its own two dates like a drawing does (🔒 YAZ-1834), as two
 * attributes on the root `<mxfile>` — `yaseendraw-created` / `yaseendraw-updated`, epoch
 * milliseconds exactly like the Excalidraw block — written FIRST in the tag so the tree reads them
 * off the file head (`BOARD_META_HEAD_BYTES`). draw.io drops attributes it does not know when it
 * serialises, so main re-stamps them on every save from the file's own head.
 */
...
export type DiagramRoot = 'mxfile' | 'mxGraphModel'
...
const ATTR = (name: string) => new RegExp(`\\s${name}\\s*=\\s*"([^"]*)"`)
 ...
 */
export function parseDiagramMetaAttrs(head: string): BoardMeta | null {
  const tag = mxfileOpenTag(head)
  if (tag === null) return null
  const text = head.slice(tag.start, tag.end)
  const created = Number(ATTR(DIAGRAM_CREATED_ATTR).exec(text)?.[1] ?? NaN)
  const updated = Number(ATTR(DIAGRAM_UPDATED_ATTR).exec(text)?.[1] ?? NaN)
...
export function stampDiagramMeta(xml: string, at: { createdAt: number; updatedAt: number }, prior: BoardMeta | null = null): string {
  ...
  for (const name of [DIAGRAM_CREATED_ATTR, DIAGRAM_UPDATED_ATTR]) rest = rest.replace(new RegExp(`\\s${name}\\s*=\\s*"[^"]*"`, 'g'), '')
