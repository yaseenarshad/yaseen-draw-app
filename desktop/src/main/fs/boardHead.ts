...
import { BOARD_META_HEAD_BYTES, readBoardMetaHead, type BoardMetaBlock } from '@shared/drawingAssets'
 ...
 * A missing file answers null; any other failure propagates for the caller to classify.
 */
export async function readBoardHead(file: string): Promise<BoardHead | null> {
  ...
  try {
    ...
    return { block: readBoardMetaHead(buffer.subarray(0, bytesRead).toString('utf8')), mtime: st.mtimeMs, size: st.size }
  } finally {
    await handle.close()
  }
}
