 ...
 * file on disk and the title on screen, so changing it asks first. Moves stay silent (a confirm
 * on every drag would be hostile). The LOCKED copy lives here, `deleteConfirmMessage`'s idiom,
 * so the component stays trivial.
 */
import { useEffect, useRef, type KeyboardEvent as ReactKeyboardEvent } from 'react'
import { basename, stripExt } from '../lib/paths'

/** The LOCKED copy (⚡ YAZ-888): both spellings, and no promise about anything else. */
export function renameConfirmMessage(oldName: string, newName: string): string {
...
/**
 * In-app confirm sheet for a NAME change (⚡ YAZ-888). A MIRROR of `ConfirmDelete` — which
 ...
 *
 * The same two omissions as the delete sheet, for the same reasons: the confirm button is NOT
 * `--danger` (nothing is destroyed), and there is no "Don't ask me again" (a rename is exactly
 * the gesture that earns a beat).
 *
 ...
 */
export function ConfirmRename({ oldPath, newPath, onConfirm, onCancel }: ConfirmRenameProps) {
    ...
    <div className="confirm-overlay" onMouseDown={onCancel} onKeyDown={onKeyDown}>
      <div className="confirm" role="dialog" aria-modal="true" aria-labelledby="confirm-rename-text" onMouseDown={(e) => e.stopPropagation()}>
        <p className="confirm__text" id="confirm-rename-text">
          {renameConfirmMessage(pageName(oldPath), pageName(newPath))}
        </p>
